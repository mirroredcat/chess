...
class Board
  ...
  def move_pieces(start_pos, end_pos)
    ...
    raise ArgumentError.new("There is no piece at #{start_pos}") if self[start_pos].empty?
    raise ArgumentError.new("Piece cannot move at #{end_pos}") if !self[start_pos].valid_moves.include?(end_pos)
  ...
  def deep_dup
    new_board = Board.new
    new_board.rows = @rows.map {|row|row.map do |el| 
      !el.is_a?(NullPiece) ? duplicate = el.dup : duplicate = NullPiece.instance
